...
import { Clock, Fuel, DollarSign, CreditCard, FileText, Plus, Trash2, Printer, Wallet, ExternalLink, Building2, Check, ChevronDown, ChevronUp, ShoppingBag, AlertTriangle, Info } from 'lucide-react'
...
export default function CloseShiftPage() {
  ...
  const [fuelPrices, setFuelPrices] = useState<Record<string, number>>({}) // fuelId -> price
  ...
  const [pumperMonthlyAdvances, setPumperMonthlyAdvances] = useState<Record<string, number>>({}) // pumperName -> total advances taken this month
    ...
    const loadData = async () => {
      try {
        ...
        if (!pumpersRes.ok) {
          ...
          await fetchPumperMonthlyAdvances()
        }
      } catch {
  ...
  // Calculate breakdowns whenever dependencies change
  useEffect(() => {
    // Only calculate if we have data
    if (!assignments.length && !shopAssignment) return

    const basicBreakdowns = calculateBreakdowns({
      assignments,
      shopAssignment,
      shopClosingStocks,
      pumperDeclaredCash,
      posSlips, // Use POS slips for calculation
      pumperDeclaredCreditAmounts,
      pumperDeclaredCheques,
      prices: fuelPrices
    })

    // Enrich with local state (advances, expenses, test pours) and recalculate totals
    const enrichedBreakdowns = basicBreakdowns.map(b => {
      // Find expense/advance/test pour data using pumper name
      // Note: calculateBreakdowns returns pumperName. We need to match with state.
      // State keys are also pumperName.

      const expenses = pumperExpenses[b.pumperName] || []
      const advanceTaken = pumperAdvances[b.pumperName] || 0


      const bankDeposits = expenses.filter(e => e.type === 'BANK_DEPOSIT').reduce((sum, e) => sum + (e.amount || 0), 0)

      // Update Declared Amount:
      // Total Declared = Calculated declared (cash+card+credit+cheque) + Advance + Bank Deposits 

      const totalDeclared = b.declaredAmount + advanceTaken + bankDeposits

      const variance = (b.calculatedSales || 0) - totalDeclared

      // Determine variance status
      // If variance is positive (Sales > Declared), it's Shortage -> DEDUCT
      // If variance is negative (Sales < Declared), it's Excess -> ADD
      // Wait, standard variance = Sales - Declared.
      // If Sales 1000, Declared 900 -> Variance 100 (Shortage).
      // If Sales 1000, Declared 1100 -> Variance -100 (Excess).

      const varianceStatus = variance > 20
        ? 'DEDUCT_FROM_SALARY'
        : variance < -20
          ? 'ADD_TO_SALARY'
          : 'NORMAL'
      ...
      return {
        ...
        expenses,
        advanceTaken,
        // We cannot add fields not in PumperBreakdown interface.
        // But PumperBreakdown has 'expenses' and 'advanceTaken'.
        // It does NOT have 'testPours'. logic in map above uses independent state for rendering test pours.
    ...
    setPumperBreakdowns(enrichedBreakdowns)
  }, [
    assignments,
    shopAssignment,
    shopClosingStocks,
  ...
  // Fetch monthly advances for all pumpers
  const fetchPumperMonthlyAdvances = async () => {
    try {
      // We don't need to pass pumpers array since the API returns a map by name
      const res = await fetch(`/api/pumpers/advances?date=${new Date().toISOString()}`)
      if (res.ok) {
        const advances = await res.json()
        setPumperMonthlyAdvances(advances)
      }
    } catch (err) {
      console.error('Failed to fetch pumper monthly advances:', err)
    }
  }
  ...
  // Load fuel prices for assignments (cached)
  useEffect(() => {
    if (assignments.length > 0 && selectedStation) {
      const loadPrices = async () => {
        // Get all unique fuel IDs from assignments
        const fuelIds = [...new Set(assignments.map(a => a.nozzle?.tank?.fuelId).filter(Boolean) as string[])]

        // Only fetch prices we don't have yet
        // We use a functional check inside to avoid dependency on fuelPrices
        const neededFuelIds = fuelIds.filter(id => !fuelPrices[id])

        if (neededFuelIds.length === 0) return


        const newPrices: Record<string, number> = {}
        await Promise.all(
          neededFuelIds.map(async (fuelId) => {
            try {
              const res = await fetch(`/api/prices?stationId=${selectedStation}&fuelId=${fuelId}`)
              if (res.ok) {
                const data = await res.json()
                let price = 470 // Default fallback

                if (data && !Array.isArray(data) && data.price) {
                  price = data.price
                } else if (Array.isArray(data) && data.length > 0) {
                  // Find active price with latest effective date
                  const activePrices = data.filter((p: { isActive: boolean; effectiveDate: string; price: number }) => p.isActive)
                    .sort((a: { effectiveDate: string }, b: { effectiveDate: string }) => new Date(b.effectiveDate).getTime() - new Date(a.effectiveDate).getTime())
                  price = activePrices[0]?.price || 470
                }
                newPrices[fuelId] = price
              }
            } catch (err) {
              console.error(`Error fetching price for ${fuelId}:`, err)
            }
          })
        )
  ...
  }, [assignments, selectedStation, fuelPrices]) // dependent on fuelPrices to know what to fetch, but logic prevents loop
    ...
    if (selectedShift && (assignments.length > 0 || shopAssignment) && selectedStation) {
      const calculatePumperBreakdown = async () => {
        try {
          ...
          // Use cached fuel prices
          const priceMap = fuelPrices
          ...
          pumperMap.forEach((pumperAssignments, pumperName) => {
            ...
            // Deduct test pours from meter sales
            // If fuel was returned to tank, it shouldn't be counted as a sale for the pumper
            // But the meter advanced, so we subtract it from the calculated meter sales
            const testPours = pumperTestPours[pumperName] || []
            const totalTestPourValue = testPours.reduce((sum, tp) => {
              // Only deduct if returned to tank (otherwise it's a sale/loss that needs accounting)
              // Actually, if it's a test pour, it's usually not a sale to a customer.
              // If returned -> deduct from sales.
              // If NOT returned -> it's a loss/expense? Or pumper pays?
              // Usually test pours are returned. If not, it might be "Fuel Testing Expense".
              // For now, let's assume all test pours should be deducted from "Sales Revenue" expected from Pumper.
              // If not returned, it should be an Expense.
              if (tp.returned && tp.amount > 0 && tp.nozzleId) {
                const nozzle = assignments.find(a => a.nozzleId === tp.nozzleId)?.nozzle
                const fuelId = nozzle?.tank?.fuelId
                const price = fuelId ? (priceMap[fuelId] || 470) : 470
                return sum + (tp.amount * price)
              }
              return sum
            }, 0)

            meterSales = Math.max(0, meterSales - totalTestPourValue)
            ...
            // Calculate declared amount
            // Total Declared = Cash (handed over) + Card + Credit + Cheques + Bank Deposits + Advance Taken
            // If a pumper sold 27,000:
            // - 7,000 Card
            // - 2,000 Advance
            // - 18,000 Cash (should be handed over)
            // Total = 18,000 + 7,000 + 2,000 = 27,000
            const declaredAmount = cash + totalCard + totalCredit + totalCheque + totalBankDeposits + advance

            const effectiveDeclaredAmount = declaredAmount + totalOtherExpenses
            const variance = effectiveDeclaredAmount - calculatedSales // Variance = Actual - Expected (Positive = Surplus, Negative = Shortage)

            // Status based on variance logic
            let varianceStatus: 'NORMAL' | 'ADD_TO_SALARY' | 'DEDUCT_FROM_SALARY' = 'NORMAL'
            if (variance < -20) varianceStatus = 'DEDUCT_FROM_SALARY' // Shortage > 20
            else if (variance > 20) varianceStatus = 'ADD_TO_SALARY'  // Surplus > 20
  ...
  }, [selectedShift, assignments, selectedStation, fuelPrices, pumperDeclaredCash, pumperDeclaredCardAmounts, pumperDeclaredCreditAmounts, pumperDeclaredCheques, pumperAdvances, otherPumperAdvances, pumperExpenses, posSlips, missingSlips, shopAssignment, totalShopRevenue, pumperTestPours])
  ...
  const getDuplicateCardDetails = (pumperName: string, lastFourDigits: string, cardType: string, terminalId: string, timestamp: Date, slipIdToExclude?: string): POSSlipEntry | null => {
    // Check for duplicate: same last 4 digits + same card type + same terminal + same time (down to the minute)
    const slips = posSlips[pumperName] || []

    // Helper to get time in minutes
    const getTimeInMinutes = (date: Date | string) => {
      const d = new Date(date)
      d.setSeconds(0, 0)
      return d.getTime()
    }

    const targetTime = getTimeInMinutes(timestamp)
    ...
    const duplicate = slips.find(slip => {
      ...
      return slip.lastFourDigits === lastFourDigits
        ...
        && getTimeInMinutes(slip.timestamp) === targetTime
    })
  ...
  const handleUpdatePOSSlip = (pumperName: string, slipId: string, field: keyof POSSlipEntry, value: string | number | Date) => {
    ...
    if ((field === 'lastFourDigits' || field === 'cardType' || field === 'terminalId' || field === 'timestamp')
      ...
      if (duplicateSlip) {
        ...
        setError(`ΓÜá∩╕Å FRAUD ALERT: Card ending in ${updatedSlip.lastFourDigits} (${updatedSlip.cardType}) was already added today at ${duplicateTime} for Rs. ${(duplicateSlip.amount || 0).toLocaleString()} on ${terminal?.name || 'same terminal'}. The same card with same type cannot be used twice on the same terminal at the same time!`)
  ...
  const handleCloseShiftAndPrint = async () => {
    ...
    // Validate test pours have reason
    for (const [pumperName, pours] of Object.entries(pumperTestPours)) {
      for (const pour of pours) {
        if (pour.amount > 0 && !pour.reason) {
          setError(`Please select a reason for test pour of ${pour.amount}L (Pumper: ${pumperName})`)
          return
        }
      }
    }

    // Validate duplicate POS slips
    if (Object.keys(duplicateCardErrors).length > 0) {
      setError('ΓÜá∩╕Å Cannot close shift: There are duplicate POS slips detected. Please resolve all fraud alerts first.')
      return
    }
    ...
    try {
      ...
      console.log('≡ƒöì FRONTEND DEBUG:')
                  ...
                  if (nozzleRes.ok) {
                    ...
                    if (fallbackTankId) {
                      ...
                      if (!testPourRes.ok) {
                        ...
                        console.log(`Γ£à Test pour created successfully:`, createdTestPour)
                      }
                      continue
                    }
                  }
          ...
          <div className="space-y-4">
            <Alert className="border-orange-500/20 dark:border-orange-500/30 bg-orange-500/10 dark:bg-orange-500/20">
              <AlertDescription className="text-orange-700 dark:text-orange-300">
                ...
                If variance is less than -Rs. 20, deduct from pumper&apos;s salary. Variance within &plusmn;Rs. 20 is normal.
              </AlertDescription>
            </Alert>
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/30 p-3 rounded-lg border space-y-2">
                      <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1 block">Calculated Sales:</span>
                    ...
                    <div className="space-y-4">
                      <span className="text-sm font-medium mb-3 block">Declared Amount Breakdown:</span>

                      {/* Cash Amount */}
                      <div className="space-y-2">
                        <Label className="flex items-center gap-2" htmlFor={`cash-amount-${breakdown.pumperName}`}>
                          <DollarSign className="h-4 w-4" />
                          Cash Handed Over (Net)
                        </Label>
                        <p className="text-xs text-muted-foreground mb-2">
                          Enter the physical cash you are handing over. Do NOT include advances (they are added automatically).
                        </p>
                        <MoneyInput
                          id={`cash-amount-${breakdown.pumperName}`}
                                        ...
                                        <div key={slip.id} className={`border rounded-lg p-3 space-y-3 bg-orange-500/5 dark:bg-orange-500/10 ${isMinimized ? 'cursor-pointer hover:bg-orange-500/10 dark:hover:bg-orange-500/15' : ''}`}>
                                            ...
                                            {isMinimized ? (
                                              ...
                                              >
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <span className="text-sm font-medium">POS Slip - {slip.lastFourDigits || 'Incomplete'}</span>
                                              ...
                                              <span className="text-sm font-medium">POS Slip</span>
                                            )}
                                            <div className="flex items-center gap-2">
                                          ...
                                          {!isMinimized && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                              <div>
                                                <Label htmlFor={`pos-machine-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">POS Machine *</Label>
                                                <Select
                                                  value={slip.terminalId}
                                                  onValueChange={(value) => handleUpdatePOSSlip(breakdown.pumperName, slip.id, 'terminalId', value)}
                                                >
                                                  <SelectTrigger id={`pos-machine-${slip.id}`} className="w-full">
                                              ...
                                              <div>
                                                <Label htmlFor={`card-time-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Card Sale Time *</Label>
                                                <div id={`card-time-${slip.id}`}>
                                                  <DateTimePicker
                                              ...
                                              <div>
                                                <Label htmlFor={`card-number-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Card Number (Last 4 Digits) *</Label>
                                                <Input
                                                  id={`card-number-${slip.id}`}
                                              ...
                                              <div>
                                                <Label htmlFor={`card-type-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Card Type *</Label>
                                                <Select
                                                  value={slip.cardType}
                                                  onValueChange={(value) => handleUpdatePOSSlip(breakdown.pumperName, slip.id, 'cardType', value)}
                                                >
                                                  <SelectTrigger id={`card-type-${slip.id}`} className="w-full">
                                                    <SelectValue />
                                                  </SelectTrigger>
                                                  <SelectContent>
                                              ...
                                              <div>
                                                <Label htmlFor={`card-amount-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Amount *</Label>
                                                <MoneyInput
                                                  id={`card-amount-${slip.id}`}
                                                  value={slip.amount}
                                              ...
                                              <div>
                                                <Label htmlFor={`card-notes-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Notes (Optional)</Label>
                                                <Input
                                                  id={`card-notes-${slip.id}`}
                                                  value={slip.notes || ''}
                                              ...
                                              return (
                                                <div key={slip.id} className={`border rounded-lg p-3 space-y-3 bg-orange-500/5 dark:bg-orange-500/10 ${isMinimized ? 'cursor-pointer hover:bg-orange-500/10 dark:hover:bg-orange-500/15' : ''}`}>
                                                  ...
                                                  {!isMinimized && (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                      <div>
                                                        <Label htmlFor={`missing-terminal-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">POS Machine *</Label>
                                                        <Select
                                                          value={slip.terminalId}
                                                          onValueChange={(value) => handleUpdateMissingSlip(breakdown.pumperName, terminalId, slip.id, 'terminalId', value)}
                                                        >
                                                          <SelectTrigger id={`missing-terminal-${slip.id}`} className="w-full">
                                                            <SelectValue placeholder="Select POS machine" />
                                                          </SelectTrigger>
                                                          <SelectContent>
                                                            {posTerminals.map(terminal => (
                                                      ...
                                                      <div>
                                                        <Label htmlFor={`missing-time-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Transaction Time *</Label>
                                                        <div id={`missing-time-${slip.id}`}>
                                                          <DateTimePicker
                                                            value={slip.timestamp}
                                                      ...
                                                      <div>
                                                        <Label htmlFor={`missing-card-num-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Card Number (Last 4 Digits) *</Label>
                                                        <Input
                                                          id={`missing-card-num-${slip.id}`}
                                                          value={slip.lastFourDigits}
                                                          onChange={(e) => {
                                                            const value = e.target.value.replace(/\D/g, '').slice(0, 4)
                                                      ...
                                                      <div>
                                                        <Label htmlFor={`missing-amount-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Amount *</Label>
                                                        <MoneyInput
                                                          id={`missing-amount-${slip.id}`}
                                                          value={slip.amount}
                                                      ...
                                                      <div>
                                                        <Label htmlFor={`missing-notes-${slip.id}`} className="text-xs text-muted-foreground mb-1 block">Notes (Optional)</Label>
                                                        <Input
                                                          id={`missing-notes-${slip.id}`}
                                                          value={slip.notes || ''}
                          ...
                          <div className="space-y-3">
                            {breakdown.cheques.map((cheque) => (
                              <div key={cheque.id} className="border rounded-lg p-3 space-y-3">
                                <div className="flex items-center justify-between">
                                  <span className="text-sm font-medium">Cheque Details</span>
                                  <Button
                                    type="button"
                                    variant="outline"
                                    size="sm"
                                ...
                                <div>
                                  <Label htmlFor={`cheque-number-${cheque.id}`} className="text-xs text-muted-foreground mb-1 block">Cheque Number *</Label>
                                  <Input
                                    id={`cheque-number-${cheque.id}`}
                                ...
                                <div>
                                  <Label htmlFor={`cheque-from-${cheque.id}`} className="text-xs text-muted-foreground mb-1 block">Received From (Who Gave the Cheque) *</Label>
                                  <Input
                                    id={`cheque-from-${cheque.id}`}
                                ...
                                <div>
                                  <div>
                                    <div className="flex items-center justify-between mb-1">
                                      <Label htmlFor={`cheque-bank-${cheque.id}`} className="text-xs text-muted-foreground">Bank *</Label>
                                    ...
                                    >
                                      <SelectTrigger id={`cheque-bank-${cheque.id}`} className="w-full">
                                ...
                                <div>
                                  <Label htmlFor={`cheque-date-${cheque.id}`} className="text-xs text-muted-foreground mb-1 block">Cheque Date *</Label>
                                  <div id={`cheque-date-${cheque.id}`}>
                                    <DateTimePicker
                                ...
                                <div>
                                  <Label htmlFor={`cheque-amount-${cheque.id}`} className="text-xs text-muted-foreground mb-1 block">Amount *</Label>
                                  <MoneyInput
                                    id={`cheque-amount-${cheque.id}`}
                                    value={cheque.amount}
                            ...
                            {pumperTestPours[breakdown.pumperName].map((testPour) => {
                              ...
                              return (
                                <div key={testPour.id} className="border rounded-lg p-3 space-y-3">
                                  <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium">Test Pour</span>
                                    <Button
                                      variant="outline"
                                      size="sm"
                                  ...
                                  <div>
                                    <Label htmlFor={`test-nozzle-${testPour.id}`} className="text-xs text-muted-foreground mb-1 block">Nozzle *</Label>
                                    <Select
                                      value={testPour.nozzleId || ''}
                                      onValueChange={(value) => handleUpdatePumperTestPour(breakdown.pumperName, testPour.id, 'nozzleId', value)}
                                    >
                                      <SelectTrigger id={`test-nozzle-${testPour.id}`} className="w-full">
                                      ...
                                      <SelectContent>
                                        {assignedNozzles.length === 0 ? (
                                          ...
                                          assignedNozzles.map((nozzle) => {
                                            ...
                                            // const pumpNumber = nozzle.pump?.pumpNumber || '?' // Unused now
                                            const display = getNozzleDisplayWithBadge({
                                              id: nozzle.id,
                                  ...
                                  <div>
                                    <Label htmlFor={`test-amount-${testPour.id}`} className="text-xs text-muted-foreground mb-1 block">Amount (L) *</Label>
                                    <Input
                                      id={`test-amount-${testPour.id}`}
                                      type="number"
                                  ...
                                  <div>
                                    <Label htmlFor={`test-reason-${testPour.id}`} className="text-xs text-muted-foreground mb-1 block">Reason *</Label>
                                    <Select
                                      value={testPour.reason || ''}
                                      onValueChange={(value) => handleUpdatePumperTestPour(breakdown.pumperName, testPour.id, 'reason', value)}
                                    >
                                      <SelectTrigger id={`test-reason-${testPour.id}`} className="w-full">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <Label htmlFor={`test-returned-${testPour.id}`} className="text-xs text-muted-foreground cursor-pointer">
                                      Fuel returned to tank
                                    </Label>
                                  </div>

                                  <div>
                                    <Label htmlFor={`test-notes-${testPour.id}`} className="text-xs text-muted-foreground mb-1 block">Notes (Optional)</Label>
                                    <Input
                                      id={`test-notes-${testPour.id}`}
                      ...
                      {/* Advance Taken */}
                      <div className="space-y-2">
                        <Label htmlFor={`advance-taken-${breakdown.pumperName}`} className="flex items-center gap-2">
                          <Wallet className="h-4 w-4" />
                          Advance Taken from Sales
                        </Label>
                        <MoneyInput
                          id={`advance-taken-${breakdown.pumperName}`}
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          Money taken as advance FROM CASH SALES. Advance is tracked separately and deducted from salary. It does NOT affect variance.
                          {(() => {
                            const pumper = pumpers.find(p => p.name === breakdown.pumperName)
                            const pumperId = pumper?.id
                            const monthlyRental = pumperId ? (pumperMonthlyRentals[pumperId] || 0) : 0
                            const currentMonthAdvances = pumperMonthlyAdvances[breakdown.pumperName] || 0
                            const ADVANCE_LIMIT = pumper?.advanceLimit || 50000
                            // Available limit = Total Limit - Monthly Rental - Advances Already Taken This Month
                            const availableLimit = Math.max(0, ADVANCE_LIMIT - monthlyRental - currentMonthAdvances)

                            return (
                              <>
                                <span className="block mt-1 font-medium text-blue-600 dark:text-blue-400">
                                  Available Advance Limit: Rs. {(availableLimit || 0).toLocaleString()}
                                  {monthlyRental > 0 && <span className="text-muted-foreground font-normal"> (Rental: Rs. {(monthlyRental || 0).toLocaleString()})</span>}
                                  {currentMonthAdvances > 0 && <span className="text-muted-foreground font-normal"> (Used: Rs. {(currentMonthAdvances || 0).toLocaleString()})</span>}
                                </span>

                                {breakdown.advanceTaken > 0 && breakdown.declaredCash >= 0 && (
                                  <span className="block mt-1 text-orange-600 dark:text-orange-400">
                                    Total Cash Generated (Collected): Rs. {(breakdown.declaredCash + (breakdown.advanceTaken) || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                  </span>
                                )}
                                {breakdown.advanceTaken > availableLimit && (
                                  <span className="flex items-center mt-1 text-red-600 dark:text-red-400 font-bold">
                                    <AlertTriangle className="h-3 w-3 mr-1" /> Limit Exceeded: Max advance is Rs. {(availableLimit || 0).toLocaleString()}
                                  </span>
                                )}
                              </>
                            )
                              ...
                              {otherAdvances.length > 0 && (
                                <div className="pt-2 border-t space-y-1">
                                  ...
                                  {totalAdvances > availableCash && (
                                    <p className="flex items-center text-xs text-red-600 dark:text-red-400 font-medium mt-1">
                                      <AlertTriangle className="h-3 w-3 mr-1" /> Warning: Total advances exceed cash declared. Advances can only come from cash.
                                    </p>
                                  )}
                                </div>
                              )}
                      ...
                      <div className="pt-4 border-t space-y-2 bg-muted/30 dark:bg-muted/20 p-3 rounded-lg">
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div className="text-muted-foreground">Cash Handed Over:</div>
                            ...
                            {(() => {
                              ...
                              if (totalAdvances > 0) {
                                return (
                                  <>
                                    <div className="text-muted-foreground text-orange-600 dark:text-orange-400">Advance Taken (add to sales):</div>
                                    ...
                                    {otherAdvancesTotal > 0 && (
                                      <>
                                        <div className="text-muted-foreground text-orange-600 dark:text-orange-400">Advances Given (add to sales):</div>
                                        <div className="font-mono text-right text-orange-600 dark:text-orange-400">
                                          + Rs. {(otherAdvancesTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </div>
                                      </>
                                    )}
                                    <div className="text-muted-foreground font-medium border-t pt-1 mt-1">Total Cash Sales:</div>
                                    <div className="font-mono text-right font-medium border-t pt-1 mt-1">
                          ...
                          <div className="flex justify-between items-center pt-2 border-t mt-2">
                            ...
                            <div className="text-xs text-muted-foreground">
                              (Cash {(breakdown.declaredCash || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} + Advance + Card + Credit + Cheque + Bank Deposits)
                            </div>
                          </div>
                        </div>
                              ...
                              {breakdown.advanceTaken > 0 && (
                                <div className="mb-2 p-2 bg-orange-500/10 dark:bg-orange-500/20 rounded border border-orange-500/20">
                                  ...
                                  <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                                    <Info className="h-3 w-3 inline mr-1" /> Note: Advance is tracked separately and will be deducted from salary. It does NOT affect variance calculation.
                                  </p>
                                </div>
                              )}
                  ...
                  <div className="mt-4 pt-4 border-t">
                    ...
                    <div className="space-y-2">
                      {breakdown.assignments.map((assignment) => {
                        ...
                        let nozzleDisplay = 'Unknown Nozzle'
                        let fuelType = 'Unknown'

                        if (assignment.nozzle) {
                          const display = getNozzleDisplayWithBadge({
                            id: assignment.nozzle.id,
                            pumpNumber: assignment.nozzle.pump?.pumpNumber || '?',
                            nozzleNumber: assignment.nozzle.nozzleNumber,
                            fuelType: assignment.nozzle.tank?.fuel?.name || 'Unknown'
                          })
                          nozzleDisplay = display.label
                          fuelType = display.badge
                        }
