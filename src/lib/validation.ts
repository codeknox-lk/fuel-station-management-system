 ...
 * Provides safe parsing and validation functions
 */
...
/**
 * Validate amount allowing zero (for optional amounts)
 * Returns error message if invalid, null if valid
 */
export function validateAmountAllowZero(amount: number | string | undefined | null, fieldName: string = 'Amount'): string | null {
  const numAmount = safeParseFloat(amount)
